...
export function getNodeCenter(node: Node): XY {
    return {
...
export function getIntersectionPoint(node: Node, from: XY, to: XY): XY {
    ...
    if (dx !== 0) {
        ...
        if (tLeft > 0 && yLeft >= y && yLeft <= y + h) candidates.push({ x, y: yLeft })

        const tRight = (x + w - from.x) / dx
        const yRight = from.y + tRight * dy
        if (tRight > 0 && yRight >= y && yRight <= y + h) candidates.push({ x: x + w, y: yRight })
    }
    if (dy !== 0) {
        const tTop = (y - from.y) / dy
        const xTop = from.x + tTop * dx
        if (tTop > 0 && xTop >= x && xTop <= x + w) candidates.push({ x: xTop, y })

        const tBottom = (y + h - from.y) / dy
        const xBottom = from.x + tBottom * dx
        if (tBottom > 0 && xBottom >= x && xBottom <= x + w) candidates.push({ x: xBottom, y: y + h })
    }
